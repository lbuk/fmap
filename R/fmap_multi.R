...
fmap_multi = function(ncircles, radius_inner = NULL, radius_outer = NULL, geo_points, geo_centres, id_var = NULL, sum = NULL, mean = NULL, median = NULL, count = F, output = 'plot') {
  ...
  if(grepl(x = class(geo_points)[1], pattern = "sf", ignore.case = T) != T && grepl(x = class(geo_points)[1], pattern = "sp", ignore.case = T) != T) {
    ...
    geo_points = geo_points %>% sf::st_as_sf()
  ...
  fmaps = lapply(1:nrow(geo_centres), function(i) {
    ...
    geo_points = geo_points %>% sf::st_transform(crs_aeqd)
    ...
    if(is.null(sum) && is.null(mean) && is.null(median) && count != T) {
      ...
      fmaps = fcircles %>%
        dplyr::mutate(circle_count = lengths(st_intersects(., geo_points)), id = id, title = "Count") %>%
        sf::st_transform(crs) %>%
      ...
      fmaps = fcircles %>%
        ...
        sf::st_transform(crs) %>%
        dplyr::mutate(id = id, title = paste0("Total ", '("', sum, '")')) %>%
      ...
      fmaps = fcircles %>%
        ...
        sf::st_transform(crs) %>%
        dplyr::mutate(id = id, title = paste0("Mean ", '("', mean, '")')) %>%
      ...
      fmaps = fcircles %>%
        ...
        sf::st_transform(crs) %>%
        dplyr::mutate(id = id, title = paste0("Median ", '("', median, '")')) %>%
